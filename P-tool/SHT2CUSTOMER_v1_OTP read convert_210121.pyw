...
def btn_OpenFile():
    ...
    SourcePath = tkinter.filedialog.askopenfilename(title = "选择源文件",filetypes = [("text","*.jpg *.png *.bmp *.pgm *.pbm"),("All File","*")])#(*.jpg *.png *.bmp *.pgm *.pbm);;All(*.*)
    print(SourcePath)
    imgCv2 = cv2.imread(SourcePath)
    print(imgCv2)
...
btn_convert = tkinter.Button(cavas_menu,width = 10,height = 1,text = "ddd",command = btn_convert)
